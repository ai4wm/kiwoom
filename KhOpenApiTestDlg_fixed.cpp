...
BEGIN_MESSAGE_MAP(CKhOpenApiTestDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_CURRENTPRICE2,	OnBtnTimeOff)
...
//! Comment       : 
//******************************************************************/
void CKhOpenApiTestDlg::OnBtnTimeOff()
{
	if (!GetNextScreenNum(2))
	{
		return;
	...
	CTimeOffOrderDlg *pTimeOffOrderDlg = new CTimeOffOrderDlg(this);
	pTimeOffOrderDlg->m_strScrNo.Format("%04d", m_nScrN0);
	pTimeOffOrderDlg->Create(IDD_TIMEOFFODER);

	m_mapScreen.SetAt(pTimeOffOrderDlg->m_strScrNo, pTimeOffOrderDlg);
	pTimeOffOrderDlg->ShowWindow(SW_SHOW);
}
